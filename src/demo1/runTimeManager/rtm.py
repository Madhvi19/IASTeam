...
class InputHandler(threading.Thread):
    ...
    def run(self):
        consumer=KafkaConsumer(intputTopic,bootstrap_servers=['localhost:9092'],enable_auto_commit=True,group_id='rtm',value_deserializer=lambda x: json.loads(x.decode('utf-8')))
...
class MonitorHandeler(threading.Thread):
    ...
    def run(self):
        consumer=KafkaConsumer(monitorTopic,bootstrap_servers=['localhost:9092'],enable_auto_commit=True,group_id='jok',value_deserializer=lambda x: json.loads(x.decode('utf-8')))
        producer=KafkaProducer(bootstrap_servers=['localhost:9092'],value_serializer=lambda x:json.dumps(x).encode('utf-8'))
        for message in consumer:
